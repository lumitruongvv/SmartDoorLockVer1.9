 ...
 * @retval  
 */
uint8_t TSM12_read_byte(uint8_t address)
{
    uint8_t val = 0x00;
 ...
 * @retval  
 */
void TSM12_write_byte(uint8_t reg_addr, uint8_t value)
{
    SW_I2C_start();
    SW_I2C_write(TSM12_WR);
...
static void TSM12_init_pin_control(void)
{
    ...
    GPIO_Init(
        TSM12_interrupt_port,(GPIO_Pin_TypeDef)TSM12_interrupt_pin, GPIO_Mode_In_FL_IT
        );
    ...
    GPIO_WriteBit(TSM12_I2C_rst_port, (GPIO_Pin_TypeDef)TSM12_I2C_rst_pin, RESET);
...
void TSM12_init(void)
{
    ...
    TSM12_write_byte(
        TSM12_CAL_HOLD_1, 0xF8
           );
    TSM12_write_byte(
        TSM12_CAL_HOLD_2, 0x01
           );
...
void app_main_handle_touch_btn_press(char btn)
{
    ...
    switch(btn)
    {
        case '0':
            ...
            AUDIO_Press_Send(SOUND_WHEN_PRESS_NUMBER, TIME_SOUND_WHEN_PRESS);
