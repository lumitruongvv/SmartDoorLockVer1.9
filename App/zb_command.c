...
volatile uint8_t zb_flagjoinnetwork, zb_flagsetlock, zb_flagsetpin;
volatile uint8_t zb_flagtogglenetwork, zb_flagchangelockstate;
volatile uint8_t zb_flagnetworkinfo, networkstatus, zb_flaggetpin;
volatile uint8_t zb_flagupdatepincode, zb_flagclearpincode, zb_flagsetfinger;
volatile uint8_t zb_getid = 1, zb_flagchangepin, zb_flaggetallpin, zb_getid_again = 1;
...
extern uint8_t Buffer_vtupdateallpincode[6];
...
static void ZB_GetPinCode(u8 type,u8 *buff, u8 len)
{
    ...
    zb_flaggetpin = 1;
    zb_getid = buff[0];
    zb_getid_again = buff[0];
}

static void ZB_SetPinCode(u8 type,u8 *buff, u8 len)
{
    zb_flagsetpin = 1;
    cleanmemory(Password_Array, 0x00, sizeof(Password_Array));
    for(uint8_t i = 0; i < len-2 ; i++)
    {
        Password_Array[i] = buff[i+2];
    }
    if(buff[0] > 25)
        zb_getid = 30;
    else
